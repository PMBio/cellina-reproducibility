...
| `ablation_loo_brain.py` | CellinaGraph sweep for the MERFISH mouse brain dataset (Isocortex → Thalamus / Hippocampus). Same holdout × loss-type × λ structure as `ablation_loo.py`. Appends to `results/ablation_loo_brain.csv`; writes `results/ablation_loo_brain.pdf`. |
| `ablation_loo_base_brain.py` | CellinaBase (no GCN) equivalent of `ablation_loo_brain.py`, varying `domain_classifier_lambda`. Appends to the same `results/ablation_loo_brain.csv`. |
...
- `results/ablation_loo.csv` — one row per (holdout cell type × spatial loss type × λ × cell type × target) for CRC
- `results/ablation_loo.pdf` / `results/ablation_loo_base.pdf` — quick diagnostic PDFs for CRC runs
- `results/ablation_loo_brain.csv` — brain rows (CellinaGraph + CellinaBase, distinguished by `spatial_loss_type`)
- `results/ablation_loo_brain.pdf` / `results/ablation_loo_base_brain.pdf` — diagnostic PDFs for brain runs
