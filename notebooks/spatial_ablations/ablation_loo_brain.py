...
MERFISH mouse brain version — analogous to ablation_loo.py but uses
load_merfish_brain() and brain-region domains instead of CRC/REF/TVA.
Results are appended to results/ablation_loo_brain.csv (shared with
ablation_loo_base_brain.py; rows distinguished by spatial_loss_type).
"""
import os
import sys
...
LINK_PREDICTION_WEIGHTS = [0, 1e-5, 0.01, 0.1, 1, 100]

MAX_EPOCHS  = 50
RESULTS_CSV = "results/ablation_loo_brain.csv"
RESULTS_PDF = "results/ablation_loo_brain.pdf"
...
def run_one(adata_base, holdout_celltype, link_prediction_weight, spatial_loss_type):
    ...
    model = CellinaModel(
        ...
        use_batch_norm=False if spatial_loss_type == "supcon" else True,
    )
