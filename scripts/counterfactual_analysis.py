...
def get_counterfactual_preds(model, adata, labels_key, model_class):
    ...
    for celltype in tqdm(adata[adata.obs['is_holdout']].obs[labels_key].cat.categories):
        ...
        adata_cf.obsm[f'{model_class}_latent'] = model.get_latent_representation(adata=adata_cf)
        adata_control.obsm[f'{model_class}_latent'] = model.get_latent_representation(adata=adata_control)
        adata_target.obsm[f'{model_class}_latent'] = model.get_latent_representation(adata=adata_target)

        if model_class == 'cellina':
            for latent_key in ['z', 's']:
                adata_cf.obsm[latent_key] = model.get_latent_representation(adata=adata_cf, latent_key=latent_key)
                adata_control.obsm[latent_key] = model.get_latent_representation(adata=adata_control, latent_key=latent_key)
                adata_target.obsm[latent_key] = model.get_latent_representation(adata=adata_target, latent_key=latent_key)
...
def get_edistances(adata_dict, model_class, n_subsample=500, n_perms=500, compute_pvals=False):
    groups = adata_dict['Endothelial'].obs["group"].unique()
    pairs = [('counterfactual', 'control'), ('counterfactual', 'target'), ('control', 'target')]
    ...
    for ct in tqdm(celltypes, desc="celltypes (E-dist)"):
        ad = adata_dict[ct]
        groups = ad.obs['group'].cat.categories if hasattr(ad.obs['group'], 'cat') else np.unique(ad.obs['group'])
        groups = list(groups)
...
def get_de_correlations(cf_adatas, k=50, eps=1e-6, method='lfc', plot=False):
    ...
    for ct, adata in tqdm(cf_adatas.items()):
        ...
        recon_all = _to_dense(adata.obsm.get('recon_x'))  # model-normalized counterfactuals (may be None)
        ...
        mean_control = X_all[mask_control].mean(axis=0) if mask_control.sum() > 0 else np.zeros(X_all.shape[1])
        mean_target  = X_all[mask_target].mean(axis=0)  if mask_target.sum()  > 0 else np.zeros(X_all.shape[1])
        mean_cf      = None
        if recon_all is not None and mask_cf.sum() > 0:
            ...
            mean_cf = X_all[mask_cf].mean(axis=0) if mask_cf.sum() > 0 else np.zeros(X_all.shape[1])
        ...
        if valid.sum() < 2 or (np.nanstd(gt_vec[valid]) == 0) or (np.nanstd(cf_vec[valid]) == 0):
            pear = np.nan
            spearman = np.nan
        else:
        ...
        results.append({'celltype': ct, 'pearson': pear, 'spearman': spearman, f'prec@{k}': prec})
    ...
    if plot and len(results) > 0:
        ...
        for i, ct in enumerate(ordered_cts):
            ...
            ax.scatter(gt_vec, cf_vec, s=10, alpha=0.45, color='gray', edgecolors='none')
            # highlight top-k GT genes
            top_idx = np.argsort(-np.abs(gt_vec))[:k]
            ax.scatter(gt_vec[top_idx], cf_vec[top_idx], s=20, color='red', alpha=0.8, label=f'top-{k} GT')
